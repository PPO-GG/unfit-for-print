...
export const useGetPlayerName = () => {
     ...
     */
    const fetchPlayerByUserId = async (userId: string): Promise<Player | null> => {
        try {
            ...
            if (res.documents.length > 0) {
                ...
                const player = {
                    ...
                    lobbyId: res.documents[0].lobbyId,
                    name: res.documents[0].name,
                    avatar: res.documents[0].avatar,
                    isHost: res.documents[0].isHost,
                    joinedAt: res.documents[0].joinedAt,
                    provider: res.documents[0].provider,
                } as Player
