...
export function useSpeech(options: TTSOptions = {}) {
    ...
    const playNext = async () => {
        if (!import.meta.client || isSpeaking.value || queue.value.length === 0) return
        ...
        try {
            if (provider === 'browser') {
                ...
                browserSpeech.speak(text, mergedOptions.browserVoice, mergedOptions.browserRate)
            ...
            if (!response.ok || !contentType.includes('audio')) {
                console.error(`TTS response error (${provider}):`, await response.text())
                isSpeaking.value = false
                await playNext()
                return
            }
