<template>
  <div class="mx-auto p-6 bg-white dark:bg-gray-900 rounded-lg shadow-md">
    ...
    <form v-if="isLoggedIn" @submit.prevent="submitCard" class="space-y-6">
      ...
      <div>
        <ClientOnly>
          <UButtonGroup v-model="cardType" class="w-full">
            <UButton 
              value="white"
              color="secondary"
              variant="soft" 
        ...
        <p v-if="cardType === 'black'" class="mt-2 text-sm text-gray-500">
          Use underscores (_) to indicate blank spaces where white cards will be placed.
        </p>
      </div>
      ...
      <ClientOnly>
        <UForm label="Card Text" name="cardText" :state="{ cardText }">
          <UTextarea
            v-model="cardText"
            :placeholder="cardType === 'white' ? 'Enter your white card text...' : 'Enter your black card text with _ for blanks...'"
            :maxlength="200"
            :rows="3"
            class="w-full"
...
async function submitCard() {
  ...
  try {
    ...
    const submissionData = {
      ...
      submitterName: userStore.user?.name || userStore.user?.prefs?.username || 'Anonymous',
