		...
		>
			<template #content>
				...
				>
					...
					<div class="sticky top-0 z-10 bg-slate-200 dark:bg-slate-900 p-3 border-b-2 border-slate-700/25">
					...
					<div class="flex-1 overflow-y-auto flex flex-col p-4 bg-slate-100 dark:bg-slate-900">
						...
						<template v-if="isAuthenticatedUser(userStore.user)">
							<ClientOnly>
								<UButton v-if="isAdmin" block class="mb-2 text-xl py-3 border-2 dark:border-none" color="error"
						...
						<div class="mt-auto flex flex-col items-center justify-center gap-2 pt-4">
							...
							<ClientOnly>
								<NuxtLink class="cursor-pointer" target="_blank" to="https://github.com/PPO-GG/unfit-for-print">
									<img alt="GitHub package.json version" class="w-12 bg-slate-600 rounded-md"
