...
export function useGameSettings() {
  ...
  const getGameSettings = async (
    ...
    try {
      ...
      if (import.meta.dev)
        console.warn("No matching game settings found for lobbyId:", lobbyId);
  ...
  const createDefaultGameSettings = async (
    ...
    if (!lobbyId)
      throw new Error("Cannot create game settings without a valid lobbyId");
    const config = useRuntimeConfig();
  ...
  const saveGameSettings = async (
    ...
    try {
      ...
      return await createDefaultGameSettings(
        lobbyId,
        newSettings.lobbyName,
        hostUserId,
      );
    } catch (err) {
