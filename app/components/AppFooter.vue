...
<template>
    ...
    >
      ...
      <div class="flex-1 flex justify-center items-center align-middle gap-2">
        <p class="text-sm h-full flex items-center">
          Made with ❤️ by MYND @ PPO.GG
        </p>
        <ClientOnly>
          <NuxtLink
            class="h-full flex items-center cursor-pointer align-middle mb-1 w-12 bg-slate-600 rounded-md"
            target="_blank"
  ...
  <UModal
    v-model:open="uiStore.showPolicy"
    :title="t('modal.privacy_policy')"
    class="m-4 rounded-lg backdrop-blur-2xl bg-slate-900/50"
    fullscreen
  >
    <template #body>
