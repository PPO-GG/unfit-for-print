...
const getPlayerName = (playerId: string) => {
  ...
  const playerByUserId = (props.players as any[]).find(
  ...
  const playerById = (props.players as any[]).find(
    (p: any) => p.$id === playerId,
  );
  if (playerById?.name) {
...
function handleScroll(event: Event) {
  ...
  activeSubmissionIndex.value = Math.max(
    0,
    Math.min(index, props.otherSubmissions.length - 1),
  );
}
</script>
...
<template>
  <div
    class="flex-1 overflow-y-auto bg-slate-700/25 border-y-2 border-b-slate-700 border-t-slate-900 rounded-3xl p-4 sm:p-6 md:p-8 text-center w-full outline-2 outline-slate-400/25"
  >
    <p
      v-if="!isJudge"
      class="text-center mb-4 sm:mb-6 text-slate-100 text-2xl sm:text-3xl font-['Bebas_Neue']"
    >
      {{ t("game.submissions") }}
    </p>
      ...
      <div class="flex-1 overflow-hidden">
        <div class="bg-slate-800/50 p-4 rounded-xl w-full h-full">
          ...
          >
            ...
            <div
              class="flex-shrink-0 w-[calc(50%-125px)] min-w-[10px] sm:min-w-[20px] md:min-w-[50px]"
            ></div>
            ...
            >
              <div
                class="flex flex-nowrap overflow-x-auto scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-slate-800 justify-start gap-2 mb-4 pb-2 max-w-full"
              >
                <WhiteCard
              ...
              >
                <span
                  class="text-white text-center w-full font-light text-lg sm:text-xl font-['Bebas_Neue']"
                >
                  {{ t("game.select_winner") }}
                </span>
              </UButton>
              ...
              <p
                v-else-if="effectiveRoundWinner === sub.playerId"
                class="text-green-400 font-bold mt-1 sm:mt-2 text-sm sm:text-base"
              >
                🏆 {{ t("game.winner") }} 🏆
              </p>
            </div>
    ...
    <div
      v-else
      class="flex flex-col items-center gap-6 max-h-[60vh] overflow-y-auto scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-transparent px-2"
    >
      <!-- Current player's submission (visible unless another player won) -->
      <div
        v-if="
          submissions[myId] &&
          (!effectiveRoundWinner || effectiveRoundWinner === myId)
        "
        class=""
      >
        <div
        ...
        >
          <p class="font-medium text-white mb-1 sm:mb-2 text-base sm:text-lg">
            <span
              class="text-success-400 font-['Bebas_Neue'] text-xl sm:text-2xl"
              >{{ t("game.your_submission") }}</span
            >
          </p>
          <div
            class="flex flex-nowrap justify-start gap-2 mb-2 pb-2 max-w-full"
          >
            <WhiteCard
          ...
          <p
            v-if="effectiveRoundWinner === myId"
            class="text-green-400 font-bold mt-1 sm:mt-2 text-sm sm:text-base"
          >
            🏆 {{ t("game.you_won") }} 🏆
          </p>
      ...
      <div
        v-if="shuffledSubmissions.length > 0"
        ...
        >
          ...
          >
            <span class="text-amber-400">{{
              t("game.winning_submission") || "Winning Submission"
            }}</span>
          </p>
          <p
            v-else
            class="font-medium text-white mb-1 sm:mb-2 text-base sm:text-lg"
          >
            <span
              class="text-amber-400 font-['Bebas_Neue'] text-xl sm:text-2xl"
              >{{ t("game.submissions") }}</span
            >
          </p>
          <div
            class="flex flex-nowrap justify-start gap-2 mb-2 pb-2 max-w-full"
      ...
      <p
        v-else
        class="text-center italic text-gray-500 mt-4 sm:mt-6 text-sm sm:text-base"
      >
