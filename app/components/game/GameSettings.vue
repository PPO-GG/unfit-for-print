    ...
    >
      <template v-if="isEditable">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <UFormField
            :label="t('game.settings.points_to_win')"
            name="maxPoints"
          >
            <UInput
          ...
          <UFormField
            :label="t('game.settings.cards_per_player')"
            name="numPlayerCards"
          >
            <UInput
...
onMounted(async () => {
  ...
  try {
    ...
    const blackCountResult = await databases.listDocuments(
      DB_ID,
      CARD_COLLECTIONS.black,
      [Query.limit(1)],
    );
    ...
    for (let offset = 0; offset < totalBlackCards; offset += chunkSize) {
      const blackCardsChunk = await databases.listDocuments(
        DB_ID,
        CARD_COLLECTIONS.black,
        [Query.limit(chunkSize), Query.offset(offset)],
      );

      blackCardsChunk.documents.forEach((card: any) => {
    ...
    const whiteCountResult = await databases.listDocuments(
      DB_ID,
      CARD_COLLECTIONS.white,
      [Query.limit(1)],
    );
    ...
    for (let offset = 0; offset < totalWhiteCards; offset += chunkSize) {
      const whiteCardsChunk = await databases.listDocuments(
        DB_ID,
        CARD_COLLECTIONS.white,
        [Query.limit(chunkSize), Query.offset(offset)],
      );

      whiteCardsChunk.documents.forEach((card: any) => {
        if (card.pack && !card.disabled) {
