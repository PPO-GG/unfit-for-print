...
**Admin Card Explorer (`/admin/cards`).** One screen: pack list · card table · inspector. The page (`app/pages/admin/cards/index.vue`) is wiring only. Rules live in pure utils — `listSelection` (click / Ctrl-⌘ / Shift, always over the on-screen order; the pack selection prunes against the whole roster, so a pack filter never deselects), `packListView` (filter, sort, group by series), `cardTableView`, `bulkField`, `explorerQuery` (URL state). `useAdminPackRoster` and `useAdminCards` load by pack id; `useExplorerMutations` owns every write, and the page reloads the roster and cards after every mutation — success or failure, since a partly failed write must not leave the view contradicting the database — instead of patching local state. `useExplorerActions` is the single list of actions read by the right-click menus, the selection bars and `defineShortcuts` — add an action there, not in a component. Editors never autosave; `dirty` drives a discard confirm before a selection change.
