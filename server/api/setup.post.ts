    ...
    for (const colConfig of appwriteConfig.collections) {
      ...
      try {
        ...
        if (e.code === 404) {
          ...
          const permissions = colConfig.$permissions.map(p => {
             ...
             if (match) {
               const command = match[1] as 'read' | 'write' | 'create' | 'update' | 'delete'
               const roleType = match[2]
               
               let role: string
               switch (roleType) {
                 case 'users': role = Role.users(); break
                 case 'guests': role = Role.guests(); break
                 case 'any': default: role = Role.any(); break
               }
      ...
      if (colConfig.attributes) {
        for (const attr of colConfig.attributes) {
          try {
            await databases.getAttribute(dbId, colConfig.$id, attr.key)
            log(`Attribute ${attr.key} in ${colConfig.name} already exists.`)
          } catch (e: any) {
            if (e.code === 404) {
              log(`Creating attribute ${attr.key} in ${colConfig.name}...`)
              switch (attr.type) {
                case 'string':
                  if (attr.format === 'enum') {
                     await databases.createEnumAttribute(dbId, colConfig.$id, attr.key, attr.elements!, attr.required, attr.default, attr.array)
                  } else if (attr.size) {
                     await databases.createStringAttribute(dbId, colConfig.$id, attr.key, attr.size, attr.required, attr.default, attr.array)
                  } else {
                     // Default size if not specified? Appwrite requires size for strings.
                     await databases.createStringAttribute(dbId, colConfig.$id, attr.key, 255, attr.required, attr.default, attr.array)
                  }
                  break
                case 'integer':
                  await databases.createIntegerAttribute(dbId, colConfig.$id, attr.key, attr.required, attr.min, attr.max, attr.default, attr.array)
                  break
                case 'boolean':
                  await databases.createBooleanAttribute(dbId, colConfig.$id, attr.key, attr.required, attr.default, attr.array)
                  break
                case 'datetime':
                  await databases.createDatetimeAttribute(dbId, colConfig.$id, attr.key, attr.required, attr.default, attr.array)
                  break
                // Add other types as needed (float, email, url, ip, etc.)
                default:
                  log(`Skipping unknown attribute type ${attr.type} for ${attr.key}`)
              }
          ...
          for (const idx of colConfig.indexes) {
              try {
                  ...
                  if (e.code === 404) {
                      ...
                      await databases.createIndex(dbId, colConfig.$id, idx.key, idx.type, idx.attributes, idx.orders)
                  }
              }
          }
      }
