  ...
  try {
    ...
    if (!state.blackDeck || state.blackDeck.length < MIN_BLACK_CARDS) {
      try {
        ...
        if (state.blackCard?.id) cardsInUse.push(state.blackCard.id);

        const availableBlackIds = allBlackIds.filter(
          (id) => !cardsInUse.includes(id),
        );
    ...
    if (!state.whiteDeck || state.whiteDeck.length < requiredWhiteCards) {
      try {
        ...
        Object.values(state.hands || {}).forEach((hand: any) => {
    ...
    console.error(`[nextRound] Error for lobby ${lobbyId}:`, err.message);
